...
public class dao {
	...
	public boolean save(modal m) { 
		...
		try {
			...
			boolean status = checkEmail( m);
			if (status) {
				System.out.println("You Are Inserting Duplicte Data");
				mYStatus=false;
			} 
			else {
				
				PreparedStatement pts = con.prepareStatement("insert into student1 (name,mobile_no,email,address)values(?,?,?,?) ");
	...
	private boolean checkEmail(modal m) {
		boolean status = false;
		try {
			Connection con = getConneton();
			PreparedStatement pts = con
					.prepareStatement("select count(*) as count from student1 where email='" + m.getEmail() + "' or mobile_no='"+m.getMobileno()+"' ");
	...
	public void updateRow(modal m) {
		try {
			...
			// System.out.print("hello im from dao class jdbc");
			pts.execute();
